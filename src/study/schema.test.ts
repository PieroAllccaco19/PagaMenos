...
describe('GRANT payload schema (spec §8.2 — reject assertedEffectiveAt before receipt lookup)', () => {
  ...
  it('rejects any other extra key and missing provenance', () => {
    ...
    expect(
      consentGrantPayloadSchema.safeParse({ consentVersion: 'cv1', optionalEvidenceConsent: true })
        .success,
    ).toBe(false);
  });
});
...
describe('WITHDRAW payload schema (spec §18)', () => {
  it('accepts empty and an assertedEffectiveAt instant', () => {
    ...
    expect(
      consentWithdrawPayloadSchema.safeParse({ assertedEffectiveAt: '2026-09-01T00:00:00.000Z' })
        .success,
    ).toBe(true);
  });
  it('rejects a non-instant asserted value and extra keys', () => {
    expect(
      consentWithdrawPayloadSchema.safeParse({ assertedEffectiveAt: 'not-a-date' }).success,
    ).toBe(false);
...
describe('participant registration input (spec §6)', () => {
  it('accepts a credential form OR a direct trusted-key form', () => {
    expect(
      registerParticipantInputSchema.safeParse({ recruitmentCredential: 'invite-abc' }).success,
    ).toBe(true);
    expect(
      registerParticipantInputSchema.safeParse({
        recruitmentSubjectKey: 'sk',
        recruitmentKeyVersion: 'v1',
      }).success,
    ).toBe(true);
  });
  it('rejects a participantCode (never caller input) and an empty payload', () => {
    expect(registerParticipantInputSchema.safeParse({ participantCode: 'PC1' }).success).toBe(
      false,
    );
