// PagaMenos · M7 V1.1 — SO-2 harness: accepted A1/A2 rows for the SO-2 real-PostgreSQL suite.
//
// Derived from the accepted S03 builder (scripts/m7/pg-m7-s03-a1a2.ts), which stays BYTE-IDENTICAL: that
// builder hard-codes `optionalEvidenceConsent: true`, and SO-2 is the first productive AGR path, so its
// suite needs participants whose GENERAL consent is valid while their optional-evidence consent is
// false. The ONLY behavioural difference is the per-participant `optionalEvidenceConsent` flag.
//
...
export interface So2A1A2Participant extends A1A2Participant {
  readonly optionalEvidenceConsent: boolean;
}

/** The same synthetic A2 build-provenance test constant as the S02 / S03 / SO-1 harnesses. */
...
/**
 * Builds `plan.length` participants, each with a GRANTED consent carrying the plan's
 * `optionalEvidenceConsent`, and `intents` finalized purchase intents with a bound decision. Must run in
 * a process whose Prisma client has not been created yet.
 */
export async function buildSo2A1A2Participants(
  databaseUrl: string,
  plan: readonly {
  ...
  let n = 0;
  const key = (p: string) => `${p}-so2vb-${String((n += 1)).padStart(4, '0')}`;
  try {
