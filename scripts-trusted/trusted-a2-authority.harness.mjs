...
function simulateRunLifecycle({
  ...
  const seed = (r) => simState.set(Number(r.id), { id: Number(r.id), name: r.name, head_sha: r.head_sha, external_id: r.external_id, status: r.status, conclusion: r.conclusion, app: r.app });
  ...
  if (!pre.ok) { events.push({ step: 'precheck', ok: false, error: pre.error }); return { events, simState }; }
  events.push({ step: 'precheck', ok: true });
  if (!appCredsPresent) { events.push({ step: 'app-auth', ok: false, error: 'TRUSTED ISSUER AVAILABILITY FAILURE' }); return { events, simState }; }
  if (listThrows) { events.push({ step: 'list', ok: false, error: listThrows }); return { events, simState }; }
  ...
  if (exact.length === 0) {
    ...
    simState.set(newId, { id: newId, name: lib.CHECK_NAME, head_sha: pre.headSha, external_id: externalId, status: 'in_progress', conclusion: null, app: { id: APP_ID } });
    activeIds = [newId];
    ...
    for (const r of exact) {
      if (failed.has(r.id)) { outcomes.push({ id: r.id, ok: false, error: 'API 500' }); continue; }
      const s = simState.get(Number(r.id));
      s.status = 'in_progress'; s.conclusion = null;               // real mutation, retained
  ...
  if (!ok) {
    ...
    for (const id of activeIds) {
      if (failed.has(id)) { outcomes.push({ id, ok: false, error: 'API 502' }); continue; }
      const s = simState.get(id);
      s.status = 'completed'; s.conclusion = 'failure'; s.name = lib.CHECK_NAME;   // retained
  ...
  for (const id of duplicateIds) {
    if (renameFailed.has(id)) { dupOutcomes.push({ id, ok: false, error: 'API 503' }); continue; }
    const s = simState.get(id);
    s.name = lib.duplicateNameFor(id);                                   // NAME leaves required context
    s.status = 'completed'; s.conclusion = 'failure';                    // retained
  ...
  s.name = lib.CHECK_NAME; s.status = 'completed'; s.conclusion = 'success';
...
function requiredContextExact({ lib, simState, externalId, headSha }) {
  const runs = Array.from(simState.values()).map((s) => ({ id: s.id, name: s.name, head_sha: s.head_sha, external_id: s.external_id, status: s.status, conclusion: s.conclusion, app: s.app }));
  return lib.findAllExactRuns({ runs, appId: APP_ID, checkName: lib.CHECK_NAME, externalId, headSha });
}
function requiredContextSuccessCount({ lib, simState, externalId, headSha }) {
  return requiredContextExact({ lib, simState, externalId, headSha }).filter((r) => r.status === 'completed' && r.conclusion === 'success').length;
}
...
async function main() {
  ...
  const foundFiltered = lib.findAllExactRuns({ runs: [...mkNonMatch, N(101, {}), N(202, {})], appId: APP_ID, checkName: lib.CHECK_NAME, externalId: eid, headSha: ACCEPTED_HEAD });
  checkRaw('§14 filter ignores wrong App/name/eid/head', foundFiltered.map((r) => r.id).sort().join(',') === '101,202');
  ...
  const dupSameId = lib.findAllExactRuns({ runs: [N(101, {}), N(101, {})], appId: APP_ID, checkName: lib.CHECK_NAME, externalId: eid, headSha: ACCEPTED_HEAD });
  checkRaw('§18 materially-identical duplicate id dedupped', dupSameId.length === 1 && dupSameId[0].id === 101);

  // §18 id-metadata conflict -> integrity failure
  let idConflictThrew = false;
  try {
    lib.findAllExactRuns({ runs: [N(101, {}), { ...N(101, {}), status: 'in_progress', conclusion: null }], appId: APP_ID, checkName: lib.CHECK_NAME, externalId: eid, headSha: ACCEPTED_HEAD });
  } catch (e) { idConflictThrew = /INTEGRITY FAILURE.*conflicting metadata/.test(e.message); }
  ...
  checkRaw('§19 canonical is numeric, not string (9 vs 100 -> 100)', canonLex && Number(canonLex.id) === 100);
  const part3 = lib.partitionCanonical([{ id: 101 }, { id: 202 }, { id: 303 }]);
  checkRaw('§3 partitionCanonical selects highest + collects rest', Number(part3.canonical.id) === 303 && part3.duplicates.map((r) => Number(r.id)).sort((a,b) => a-b).join(',') === '101,202');
  checkRaw('§4 duplicate name shape trusted-a2-authority/duplicate/<id>', lib.duplicateNameFor(101) === 'trusted-a2-authority/duplicate/101');

  // ==== §16 REAL PAGINATOR — injected fetcher exercises the loop ============
  const buildPage = (runs, isLast) => ({ ok: true, status: 200, json: { check_runs: runs } });
  const shortEnd = (runs) => buildPage(runs, true); // any chunk < PAGE_SIZE terminates
  const fullPage = (runs) => buildPage(runs); // len == PAGE_SIZE keeps going
  const mkFullPage = (start, appId = 111111) => Array.from({ length: 100 }, (_, i) => N(start + i, { appId }));
  ...
  const p2Fetcher = async (url) => { const m = /&page=(\d+)/.exec(url); return p2Pages[Number(m[1]) - 1]; };
  const p2Res = await lib.listAllCheckRunsForRefWithFetcher(p2Fetcher, paginatorInput);
  const p2Exact = lib.findAllExactRuns({ runs: p2Res, appId: APP_ID, checkName: lib.CHECK_NAME, externalId: eid, headSha: ACCEPTED_HEAD });
  checkRaw('§16 real paginator: exact match on PAGE 2 discovered (id 3200)', p2Exact.length === 1 && p2Exact[0].id === 3200);

  // page 3 discovery
  const p3Pages = [fullPage(mkFullPage(4000)), fullPage(mkFullPage(4100, 222222)), shortEnd([N(4250, {})])];
  const p3Fetcher = async (url) => { const m = /&page=(\d+)/.exec(url); return p3Pages[Number(m[1]) - 1]; };
  const p3Res = await lib.listAllCheckRunsForRefWithFetcher(p3Fetcher, paginatorInput);
  const p3Exact = lib.findAllExactRuns({ runs: p3Res, appId: APP_ID, checkName: lib.CHECK_NAME, externalId: eid, headSha: ACCEPTED_HEAD });
  checkRaw('§16 real paginator: exact match on PAGE 3 discovered (id 4250)', p3Exact.length === 1 && p3Exact[0].id === 4250);

  // API failure on page 2 -> integrity failure
  const apiFailPages = [fullPage(mkFullPage(5000)), { ok: false, status: 500, json: {} }];
  const apiFailFetcher = async (url) => { const m = /&page=(\d+)/.exec(url); return apiFailPages[Number(m[1]) - 1]; };
  let apiFailErr = null; try { await lib.listAllCheckRunsForRefWithFetcher(apiFailFetcher, paginatorInput); } catch (e) { apiFailErr = e.message; }
  checkRaw('§16 real paginator: page-2 API failure -> INTEGRITY FAILURE', apiFailErr && /INTEGRITY FAILURE/.test(apiFailErr) && /page 2/.test(apiFailErr));
  ...
  checkRaw('§16 raw-scan cap dominates MAX_PAGES under full-page load (arithmetic invariant)',
  ...
  let pageCalls = 0;
  const loopBoundFetcher = async () => { pageCalls++; return fullPage(mkFullPage(6000 + pageCalls * 100)); };
  try { await lib.listAllCheckRunsForRefWithFetcher(loopBoundFetcher, paginatorInput); } catch { /* raw-cap integrity failure expected */ }
  checkRaw('§16 real paginator: fetcher actually invoked N<MAX_PAGES times before raw-cap halt (loop executed)',
  ...
  let capCalls = 0;
  const capFetcher = async () => { capCalls++; return fullPage(mkFullPage(7000 + capCalls * 100)); };
  let capErr = null; try { await lib.listAllCheckRunsForRefWithFetcher(capFetcher, paginatorInput); } catch (e) { capErr = e.message; }
  checkRaw('§16 real paginator: raw-scan sanity ceiling crossed -> INTEGRITY FAILURE', capErr && /sanity ceiling/.test(capErr) && capCalls < lib.MAX_PAGES);

  // Response-shape failure -> integrity failure
  const badShapeFetcher = async () => ({ ok: true, status: 200, json: { not_check_runs: [] } });
  let shapeErr = null; try { await lib.listAllCheckRunsForRefWithFetcher(badShapeFetcher, paginatorInput); } catch (e) { shapeErr = e.message; }
  checkRaw('§16 real paginator: unexpected response shape -> INTEGRITY FAILURE', shapeErr && /unexpected check-runs response shape/.test(shapeErr));
  ...
  const canonPass = simulateRunLifecycle({ lib, headSha: ACCEPTED_HEAD, priorRuns: threePriorSuccess, revocableResult: { ok: true } });
  checkRaw('§10 3-way duplicate reset ALL to in_progress before validate', canonPass.events.some((e) => e.step === 'reset-all' && e.ids.length === 3));
  checkRaw('§10 canonical chosen = 303 (highest numeric)', canonPass.canonicalId === 303);
  checkRaw('§10 renamed all noncanonical [101,202] BEFORE canonical success', (() => {
  ...
  const canonPassRequired = requiredContextExact({ lib, simState: canonPass.simState, externalId: canonPass.externalId, headSha: ACCEPTED_HEAD });
  checkRaw('§6 exactly ONE required-context exact run remains (canonical)', canonPassRequired.length === 1 && Number(canonPassRequired[0].id) === 303);
  checkRaw('§6 exactly ONE required-context SUCCESS published', requiredContextSuccessCount({ lib, simState: canonPass.simState, externalId: canonPass.externalId, headSha: ACCEPTED_HEAD }) === 1);
  checkRaw('§4 noncanonical duplicates renamed to trusted-a2-authority/duplicate/<id>',
  ...
  checkRaw('§5 renamed duplicates carry non-success conclusion (failure)',
  ...
  const canonFail = simulateRunLifecycle({ lib, headSha: ACCEPTED_HEAD, priorRuns: threePriorSuccess, revocableResult: { ok: false, error: 'head pin failed' } });
  const canonFailRequired = requiredContextExact({ lib, simState: canonFail.simState, externalId: canonFail.externalId, headSha: ACCEPTED_HEAD });
  checkRaw('§11 validation FAIL: all 3 exact runs end non-success', canonFailRequired.length === 3 && canonFailRequired.every((r) => r.conclusion === 'failure'));
  checkRaw('§11 no required-context SUCCESS after validation FAIL', requiredContextSuccessCount({ lib, simState: canonFail.simState, externalId: canonFail.externalId, headSha: ACCEPTED_HEAD }) === 0);
  ...
  checkRaw('§12A canonical success PATCH failure -> integrity event emitted',
    solA.events.some((e) => e.step === 'canonical-integrity-failure'));
  checkRaw('§12A canonical 202 NEVER receives success',
    solA.simState.get(202).conclusion !== 'success');
  checkRaw('§12A NO required-context SUCCESS anywhere',
    requiredContextSuccessCount({ lib, simState: solA.simState, externalId: solA.externalId, headSha: ACCEPTED_HEAD }) === 0);
  checkRaw('§17 sequential fidelity: earlier duplicate rename PATCH RETAINED (101 stays renamed)',
  ...
  checkRaw('§12B rename integrity failure emitted before canonical success',
    solB.events.some((e) => e.step === 'rename-integrity-failure') &&
    !solB.events.some((e) => e.step === 'canonical-success'));
  checkRaw('§12B canonical 303 NEVER promoted to success',
    solB.simState.get(303).conclusion !== 'success' && solB.simState.get(303).name === lib.CHECK_NAME);
  ...
  checkRaw('§12B NO required-context SUCCESS published',
    requiredContextSuccessCount({ lib, simState: solB.simState, externalId: solB.externalId, headSha: ACCEPTED_HEAD }) === 0);
  checkRaw('§17 sequential fidelity: earlier rename of 101 RETAINED despite later 202 failure',
  ...
  const partialReset = simulateRunLifecycle({ lib, headSha: ACCEPTED_HEAD, priorRuns: threePriorSuccess, revocableResult: { ok: true }, resetFailIds: [202] });
  checkRaw('§8 partial RESET failure classified integrity + halt before validate',
  ...
  const failPathPartial = simulateRunLifecycle({ lib, headSha: ACCEPTED_HEAD, priorRuns: threePriorSuccess, revocableResult: { ok: false, error: 'authority unavailable' }, finalizeFailIds: [202] });
  checkRaw('§7 FAIL-path partial finalize classified integrity failure',
    failPathPartial.events.some((e) => e.step === 'finalize-integrity-failure'));
  checkRaw('§7 FAIL-path partial finalize: NO required-context success',
    requiredContextSuccessCount({ lib, simState: failPathPartial.simState, externalId: failPathPartial.externalId, headSha: ACCEPTED_HEAD }) === 0);
  checkRaw('§17 FAIL-path sequential fidelity: earlier successful failure-PATCH retained',
    failPathPartial.simState.get(101).status === 'completed' && failPathPartial.simState.get(101).conclusion === 'failure');

  // ==== issuer availability ================================================
  const noCreds = simulateRunLifecycle({ lib, headSha: ACCEPTED_HEAD, priorRuns: [], revocableResult: { ok: true }, appCredsPresent: false });
  checkRaw('§8 no App creds -> abort with AVAILABILITY FAILURE', noCreds.events.some((e) => e.error && e.error.includes('AVAILABILITY')));
  ...
  checkRaw('§16 pagination failure -> abort before reset', listFails.events.some((e) => e.error && e.error.includes('INTEGRITY')));
  ...
  const futureExact = lib.findAllExactRuns({ runs: futurePrior, appId: APP_ID, checkName: lib.CHECK_NAME, externalId: eid, headSha: ACCEPTED_HEAD });
  checkRaw('§15 future rerun: only canonical (303) enters exact required-name set',
    futureExact.length === 1 && Number(futureExact[0].id) === 303);
  const futureRerun = simulateRunLifecycle({ lib, headSha: ACCEPTED_HEAD, priorRuns: futurePrior, revocableResult: { ok: true } });
  checkRaw('§15 future rerun: no duplicate explosion (1 exact run, 0 duplicates renamed)',
    futureRerun.events.some((e) => e.step === 'reset-all' && e.ids.length === 1 && e.ids[0] === 303) &&
    (!futureRerun.duplicateIds || futureRerun.duplicateIds.length === 0));

  // ==== new head H2 gets its own check =====================================
  const H2 = 'a'.repeat(40);
  const newHead = simulateRunLifecycle({ lib, headSha: H2, priorRuns: threePriorSuccess, revocableResult: { ok: false, error: 'head pin failed' } });
  ...
  for (const fc of failureCauses) {
    const sim = simulateRunLifecycle({ lib, headSha: ACCEPTED_HEAD, priorRuns: threePriorSuccess, revocableResult: fc.r });
    const req = requiredContextExact({ lib, simState: sim.simState, externalId: sim.externalId, headSha: ACCEPTED_HEAD });
    checkRaw('§7 post-reset "' + fc.label + '" -> ALL exact runs failure, 0 success', req.length === 3 && req.every((r) => r.conclusion === 'failure') && requiredContextSuccessCount({ lib, simState: sim.simState, externalId: sim.externalId, headSha: ACCEPTED_HEAD }) === 0);
  }
  ...
  const dup = lib.buildDuplicateFailureBody({ id: 101, headSha: ACCEPTED_HEAD, externalId: eid, completedAt: 'z', canonicalId: 303 });
  checkRaw('duplicate body renames + non-success conclusion + notes canonical',
    dup.name === 'trusted-a2-authority/duplicate/101' &&
    dup.status === 'completed' && dup.conclusion === 'failure' &&
  ...
  checkRaw('DUPLICATE_NAME_PREFIX distinct from CHECK_NAME', lib.DUPLICATE_NAME_PREFIX.startsWith('trusted-a2-authority/duplicate/') && !lib.DUPLICATE_NAME_PREFIX.startsWith(lib.CHECK_NAME + '/') && lib.duplicateNameFor(1) !== lib.CHECK_NAME);
